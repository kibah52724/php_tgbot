...
class SaveAction
{
    ...
    public function __invoke(object $update, ConnectionService $connectionService): void
    {
        ...
        if (!$repo->checkOnExistUserData($update)) {
            $messageHelper->sendWithoutResponse($update->message->chat->id, 'Готово. Ваше обращение принято');
            $repo->saveDate($update);
        } else {
            $messageHelper->sendWithoutResponse($update->message->chat->id, 'Ваше обращение уже в обработке');
        }
    }
}